...
contract PoolUsedInfo is Ownable {
    using LibString for string;
    MapStorage private mapStorage;
    TableFactory tf;
    string constant TABLE_NAME = "t_pool_used_information2";
    constructor() public {
    ...
    event InsertPoolUsedInfo(string customerid, string hash, string owner);
    event UpdatePoolUsedInfo(string customerid, string hash, string owner);
    event PoolUsedInfoHashNotFound(string customerid, string hash, string tips);
    ...
    function insert(string memory _id,string memory _params, string memory _data,string memory _key) public onlyOwner returns(int) {
        ...
        emit InsertPoolUsedInfo(_id, _hash, _owner);
        return count;
    }
    function update(string memory _id,string memory _hash, string memory _owner) public onlyOwner returns(int) {
        ...
        if (count == 1){
            emit UpdatePoolUsedInfo(_id, _hash, _owner);
        } else {
            emit PoolUsedInfoHashNotFound(_id, _hash, "未找到记录");
        }
        return count;
    }
