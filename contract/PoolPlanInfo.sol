...
contract PoolPlanInfo is Ownable {
    using LibString for string;
    MapStorage private mapStorage;
    TableFactory tf;
    string constant TABLE_NAME = "t_pool_plan_information2";
    constructor() public {
        tf = TableFactory(0x1001);
        tf.createTable(TABLE_NAME, "id","customerid,tradeYearMonth,data,key,hash,owner");
        mapStorage = new MapStorage();
    }
    
    event InsertPoolPlanInfo(string customerid, string hash, string owner);
    event UpdatePoolPlanInfo(string customerid, string hash, string owner);
    event PoolPlanInfoHashNotFound(string customerid, string hash, string tips);
    ...
    function insert(string memory _id,string memory _params, string memory _data,string memory _key) public onlyOwner returns(int) {
        ...
        string memory _hash = ss[1];
        string memory _owner = ss[2];
        ...
        emit InsertPoolPlanInfo(_id, _hash, _owner);
        return count;
    }
    function update(string memory _id,string memory _hash, string memory _owner) public onlyOwner returns(int) {
        ...
        if (count == 1){
            emit UpdatePoolPlanInfo(_id, _hash, _owner);
        } else {
            emit PoolPlanInfoHashNotFound(_id, _hash, "未找到记录");
        }
        return count;
    }
    ...
    function select(string memory _id) private view returns(Entries _entries){
        Table table = tf.openTable(TABLE_NAME);
        require(_isProcessIdExist(table, _id), "PoolPlanInfo select: current processId not exist");
        Condition condition = table.newCondition();
