...
contract HistoryUsedInfo is Ownable {
    using LibString for string;
    MapStorage private mapStorage;
    TableFactory tf;
    string constant TABLE_NAME = "t_history_used_information3";
    constructor() public {
    ...
    event InsertUsedInfo(string customerid, string hash, string owner);
    event UpdateUsedInfo(string customerid, string hash, string owner);
    event HistoryUsedInfoHashNotFound(string customerid, string hash, string tips);
    ...
    function insert(string memory _id,string memory _params, string memory _data,string memory _key) public onlyOwner returns(int) {
        ...
        emit InsertUsedInfo(_id, _hash, _owner);
        return count;
    }
    function update(string memory _id,string memory _hash, string memory _owner) public onlyOwner returns(int) {
        ...
        if (count == 1){
            emit UpdateUsedInfo(_id, _hash, _owner);
        } else {
            emit HistoryUsedInfoHashNotFound(_id, _hash, "未找到记录");
        }
        return count;
    }
