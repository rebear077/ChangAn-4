...
contract HistoryReceivableInfo is Ownable {
    ...
    TableFactory tf;
    string constant TABLE_NAME = "t_history_receivable_information3";
    constructor() public {
    ...
    event InsertReceivableInfo(string customerid, string hash, string owner);
    event UpdateReceivableInfo(string customerid, string hash, string owner);
    event HistoryReceivableInfoHashNotFound(string customerid, string hash, string tips);
    ...
    function insert(string memory _id,string memory _params, string memory _data,string memory _key) public onlyOwner returns(int) {
        ...
        emit InsertReceivableInfo(_id, _hash, _owner);
        return count;
    }
    function update(string memory _id, string memory _hash, string memory _owner) public onlyOwner returns(int) {
        ...
        if (count == 1){
            emit UpdateReceivableInfo(_id, _hash, _owner);
        } else {
            emit HistoryReceivableInfoHashNotFound(_id, _hash, "未找到记录");
        }
        return count;
    }
    ...
    function select(string memory _id) private view returns(Entries _entries){
        Table table = tf.openTable(TABLE_NAME);
        require(_isProcessIdExist(table, _id), "HistoryReceivableInfo select: current processId not exist");
        Condition condition = table.newCondition();
