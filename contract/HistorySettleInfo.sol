...
contract HistorySettleInfo is Ownable {
    using LibString for string;
    MapStorage private mapStorage;
    TableFactory tf;
    string constant TABLE_NAME = "t_history_settle_information1";
    constructor() public {
    ...
    function select(string memory _id) private view returns(Entries _entries){
        Table table = tf.openTable(TABLE_NAME);
        require(_isProcessIdExist(table, _id), "HistorySettleInfo select: current processId not exist");
